...
class _buildPCState extends State<buildPC> {
  ...
  Future<void> _checkRecommendationServer() async {
    ...
    try {
      _recommendationServerAvailable =
          await RecommendationService.isServerHealthy();
  ...
  void _updateBuildAndCheckCompatibility() {
    setState(() {
      ...
      final finalvoltage =
          "${CompatibilityChecker.calculateTotalPowerConsumption(_currentBuild)}W";
  ...
  Future<void> _loadRecommendations() async {
    ...
    try {
      ...
      if (_selectedMotherboardIndex != null && _selectedCpuIndex == null) {
        final cpuRecs =
            await RecommendationService.getCompatibleRecommendationsSeparate(
        ...
        print(
          ' Loaded ${_cpuRecommendations.length} CPU DB + ${_cpuDatasetRecommendations.length} dataset recommendations',
        );
      } else {
        _cpuRecommendations = [];
      ...
      if (_selectedGpuIndex == null) {
        ...
        print(
          ' Loaded ${_gpuRecommendations.length} GPU DB + ${_gpuDatasetRecommendations.length} dataset recommendations',
        );
      } else {
        _gpuRecommendations = [];
      ...
      if (_selectedMotherboardIndex != null && _selectedRamIndex == null) {
        ...
        print(
          ' Loaded ${_ramRecommendations.length} RAM DB + ${_ramDatasetRecommendations.length} dataset recommendations',
        );
      } else {
        _ramRecommendations = [];
      ...
      // PSU recommendations
      if ((_selectedCpuIndex != null || _selectedGpuIndex != null) &&
        ...
        print(
          '✅ Loaded ${_psuRecommendation.length} PSU DB + ${_psuDatasetRecommendation.length} dataset recommendations',
        );
      } else {
        _psuRecommendation = [];
      ...
      if ((_selectedMotherboardIndex != null || _selectedGpuIndex != null) &&
          _selectedCaseIndex == null) {
        final caseRecs =
            await RecommendationService.getCompatibleRecommendationsSeparate(
              currentBuild: currentBuild,
              targetCategory: 'case',
        ...
        print(
          '✅ Loaded ${_caseRecommendation.length} Case DB + ${_caseDatasetRecommendation.length} dataset recommendations',
        );
      } else {
        _caseRecommendation = [];
      ...
      if (_selectedCpuIndex != null && _selectedMotherboardIndex == null) {
        ...
        print(
          ' Loaded ${_motherboardRecommendations.length} Motherboard DB + ${_motherboardDatasetRecommendations.length} dataset recommendations',
        );
      } else {
      ...
      if (_selectedStorageIndex == null) {
        ...
        print(
          ' Loaded ${_storageRecommendation.length} Storage DB + ${_storageDatasetRecommendation.length} dataset recommendations',
        );
      } else {
      ...
      if (_selectedCpuIndex != null && _selectedCoolingIndex == null) {
        ...
        print(
          ' Loaded ${_coolingRecommendation.length} Cooling DB + ${_coolingDatasetRecommendation.length} dataset recommendations',
        );
      } else {
  ...
  int? _findComponentIndexById(
    ...
    for (int i = 0; i < components.length; i++) {
      if (components[i].id == id) {
        print(
          ' Found $category ID $id at index $i: ${components[i].modelName}',
        );
        return i;
      }
    }
            ...
            recommendation.inDatabase
                ? Tooltip(
                  ...
                  child: IconButton(
                    ...
                    onPressed: () {
                      print(
                        ' Add button pressed for: ${recommendation.modelName}',
                      );
                      onSelect();
                    },
                  ),
  ...
  Future<void> _saveBuild() async {
    ...
    final compatibilityCheck = CompatibilityChecker.checkCompatibility(
      _currentBuild,
    );
      ...
      showDialog(
        ...
        builder:
            (context) => AlertDialog(
              ...
              content: Text(
                'Your Build Has Compatibility issue. Are You Sure You Wanna Save?',
    ...
    try {
      ...
      if (_currentBuild.cpu == null ||
        ...
        ScaffoldMessenger.of(context).showSnackBar(
          const SnackBar(
            content: Text(
              'One or more components are missing. Please complete your build.',
            ),
            backgroundColor: Colors.red,
          ),
        );
      ...
      if (_currentBuild.cpu!.id == null ||
        ...
        ScaffoldMessenger.of(context).showSnackBar(
          const SnackBar(
            content: Text(
              'Component IDs are invalid. Please re-select components.',
            ),
            backgroundColor: Colors.red,
          ),
        );
      ...
      if (buildId != null) {
        ScaffoldMessenger.of(context).showSnackBar(
          SnackBar(
            content: Text(
              'Build "${_buildname.text.trim()}" saved successfully!',
            ),
          ...
          (context) => AlertDialog(
            ...
            content: Column(
              ...
              children: [
                ...
                SizedBox(height: 8),
                Text(
                  'This component is from our extended dataset but not currently available in store.',
                  style: TextStyle(color: Colors.orange),
                ),
                SizedBox(height: 8),
                if (recommendation.compatibilityNotes.isNotEmpty) ...[
                  Text('Compatibility Notes:'),
                  ...recommendation.compatibilityNotes.map(
                    (note) => Text('• $note', style: TextStyle(fontSize: 12)),
                  ),
                ],
              ],
            ),
            ...
            Padding(
              ...
              child: Column(
                ...
                children: [
                  ...
                  ...databaseRecommendations.map(
                    (rec) => _recommendationItem(rec, label.toLowerCase(), () {
                      // Database items can be added
                      final foundIndex = _findComponentIndexById(
                        items,
                        rec.id,
                        label.toLowerCase(),
                      );
                      if (foundIndex != null) {
                        print(
                          '✅ Adding recommended ${label.toLowerCase()}: ${rec.modelName}',
                        );
                        ...
                        ScaffoldMessenger.of(context).showSnackBar(
                          SnackBar(
                            content: Text(
                              '${rec.modelName} added to your build!',
                            ),
                            backgroundColor: Colors.green,
                        ...
                        print(
                          ' Database component ${rec.id} not found in local list - ID mismatch?',
                        );
                        ScaffoldMessenger.of(context).showSnackBar(
                          SnackBar(
                            content: Text(
                              'Component not found in local database. Please refresh and try again.',
                            ),
                            backgroundColor: Colors.red,
                          ),
                        );
  ...
  Widget _buildCard<T>(
    ...
    return Card(
      ...
      child: ListTile(
                      ...
                      selectedIndex == null
                          ...
                          : Padding(
                            ...
                            child: Row(
                              children: [
                                _buildImageWidget(
                                  (items[selectedIndex] as dynamic).imageURL,
                                  size: 30,
                                ),
                                const SizedBox(width: 8),
                                Expanded(
        ...
        trailing:
            selectedIndex == null || items.isEmpty
                ? const Text('-')
                : Text(
                  'Rs.${(items[selectedIndex] as dynamic).price}',
  ...
  void _showDetailedCompatibility() {
    final result = CompatibilityChecker.checkCompatibility(_currentBuild);
    final voltage = CompatibilityChecker.calculateTotalPowerConsumption(
      _currentBuild,
    );
            ...
            content: SingleChildScrollView(
              child: Column(
                ...
                children: [
                  ...
                  Text(
                    'Build Completion: ${result.completionPercentage.toStringAsFixed(1)}%',
                        ...
                        child: Column(
                          ...
                          children: [
                            RadialProgressAnimation(
                              progress: _getCompatibilityProgress(),
